...
EWXWEXPORT(wxMetafileDC*,wxMetafileDC_Create)(wxString* _file)
{
	wxString file;
	
	if (_file) file = *_file;

    return new wxMetafileDC(file);
}

EWXWEXPORT(wxMetafile*,wxMetafileDC_Close)(wxMetafileDC* self)
{
	return self->Close();
}

EWXWEXPORT(void,wxMetafileDC_Delete)(wxMetafileDC* self)
{
	delete self;
}

EWXWEXPORT(wxMetafile*,wxMetafile_Create)(wxString* _file)
{
  wxString file;
  if (_file) file = *_file;
  return new wxMetafile(file);
}
