...
extern "C"
{
//EWXWCONSTANTINT(EVT_DELETE,wxEVT_DELETE)
...
//EWXWCONSTANTINT(EVT_HTML_CELL_CLICKED,wxEVT_HTML_CELL_CLICKED)
EWXWEXPORT(int,expEVT_HTML_CELL_MOUSE_HOVER)()
{
  return wxEVT_HTML_CELL_MOUSE_HOVER;
}
//EWXWCONSTANTINT(EVT_HTML_CELL_MOUSE_HOVER,wxEVT_HTML_CELL_MOUSE_HOVER)
EWXWEXPORT(int,expEVT_HTML_LINK_CLICKED)()
{
  return wxEVT_HTML_LINK_CLICKED;
}
//EWXWCONSTANTINT(EVT_HTML_LINK_CLICKED,wxEVT_HTML_LINK_CLICKED)
EWXWEXPORT(int,expEVT_HTML_SET_TITLE)()
{
  return wxEVT_HTML_SET_TITLE;
}
//EWXWCONSTANTINT(EVT_HTML_SET_TITLE,wxEVT_HTML_SET_TITLE)
EWXWEXPORT(int,expEVT_INPUT_SINK)()
{
  return wxEVT_INPUT_SINK;
}
//EWXWCONSTANTINT(EVT_INPUT_SINK,wxEVT_INPUT_SINK)
EWXWEXPORT(int,expEVT_SORT)()
{
  return wxEVT_SORT;
}
//EWXWCONSTANTINT(EVT_SORT,wxEVT_SORT)

  /* list control */
EWXWCONSTANTINT(EVT_COMMAND_LIST_CACHE_HINT,wxEVT_COMMAND_LIST_CACHE_HINT)
EWXWCONSTANTINT(EVT_COMMAND_LIST_COL_RIGHT_CLICK,wxEVT_COMMAND_LIST_COL_RIGHT_CLICK)
EWXWCONSTANTINT(EVT_COMMAND_LIST_COL_BEGIN_DRAG,wxEVT_COMMAND_LIST_COL_BEGIN_DRAG)
EWXWCONSTANTINT(EVT_COMMAND_LIST_COL_DRAGGING,wxEVT_COMMAND_LIST_COL_DRAGGING)
EWXWCONSTANTINT(EVT_COMMAND_LIST_COL_END_DRAG,wxEVT_COMMAND_LIST_COL_END_DRAG)
} /* extern "C" */
...
EWXWEXPORT(void*,wxString_Create)(wxChar* buffer)
{
  return new wxString(buffer);
}

EWXWEXPORT(void*,wxString_CreateUTF8)(utf8char* buffer)
{
  return new wxString (buffer,wxConvUTF8);
}

EWXWEXPORT(void*,wxString_CreateLen)(wxChar* buffer,int len)
{
  return new wxString(buffer,len);
}

EWXWEXPORT(void,wxString_Delete)(void* s)
{
  delete (wxString*)s;
}

EWXWEXPORT(int,wxString_GetString)(void* st,wxChar* buffer)
{
  wxString* s =(wxString*)st;
  if (buffer) memcpy (buffer, s->c_str(), s->Length() * sizeof(wxChar));
  return s->Length();
}

EWXWEXPORT(wxCharBuffer*,wxString_GetUtf8)(void* s)
{
  wxCharBuffer *cb = new wxCharBuffer;
  *cb = ((wxString*)s)->utf8_str();
  return cb;
}
...
{
  wxPoint pos;
  if (self->GetItemPosition((long)item, pos))
...
{
  return new wxString(self->GetBaseClassName1());
}

EWXWEXPORT(wxString*,wxClassInfo_GetBaseClassName2)(wxClassInfo* self)
{
  return new wxString(self->GetBaseClassName2());
}
...
{
  return new wxPoint((self)->ConvertPixelsToDialog(wxPoint(x, y)));
}

EWXWEXPORT(wxPoint*,wxWindow_ConvertDialogToPixelsEx)(wxWindow* self,int x,int y)
{
  return new wxPoint((self)->ConvertDialogToPixels(wxPoint(x, y)));
}
...
EWXWCONSTANTINT(EVT_MOUSEWHEEL,wxEVT_MOUSEWHEEL)
...
EWXWEXPORT(wxString*,wxGetPasswordFromUser)(wxString* message,wxString* caption,wxString* defaultText,wxWindow* parent)
{
  return new wxString(wxGetPasswordFromUser( *message,*caption,*defaultText, parent ));
}

EWXWEXPORT(wxString*,wxGetTextFromUser)(wxString* message,wxString* caption,wxString* defaultText,wxWindow* parent,int x,int y,bool center)
{
  return new wxString(wxGetTextFromUser( *message,*caption,*defaultText, parent, x, y, center ));
}
...
{
  return new wxPoint(self->GetLogicalPosition());
}
...
/*-----------------------------------------------------------------------------
  html window
-----------------------------------------------------------------------------*/
EWXWEXPORT(wxHtmlWindow*,wxHtmlWindow_Create)(wxWindow* _prt,int _id,long _stl,wxString* _txt)
{
    return new wxHtmlWindow(_prt, _id, wxDefaultPosition, wxDefaultSize, _stl,*_txt );
}

EWXWEXPORT(wxcHtmlWindow*,wxcHtmlWindow_Create)(wxWindow* _prt,int _id,long _stl,wxString* _txt)
{
    return new wxcHtmlWindow(_prt, _id, wxDefaultPosition, wxDefaultSize, _stl,*_txt );
}
...
EWXWEXPORT(int,wxLog_HasPendingMessages)(wxLog* self)
{
